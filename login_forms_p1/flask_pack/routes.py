...
def loginauth():
    ...
    if U_name=="admin" and P_word=="admin":
        ...
        if  specific_user_name==None  or specific_user_name.password==None:
            return render_template("login_page.html",alert="invaild user please try again")
        ...
        if specific_user_name and specific_user_password:
            ...
            return render_template("login_page.html",alert="Username or password is wrong please try again") 
...
def submitleave():
    user=User.query.filter_by(username=current_user.username).first()
    date=str(request.form['dl'])
    date1=str(request.form['dl1'])
    print(f"date  ={date} and date 1 ={date1}")
    ...
    emp=empholidays.query.filter_by(username=current_user.username).first()
    emph=er.query.filter_by(username=current_user.username).first()
    ...
    db.session.commit()
    return render_template("applyleave.html",leaves=leaves,rm=user.Reporting_Manager,msg="leave applied",value=value1)
@app.route("/attreport")
def attendence_report():
    user=User.query.filter_by(username=current_user.username).first()
    emp = employee()
    emp.data_spilt(user.Time)
    emp1=em.query.filter_by(username=current_user.username).first()
    dt=datetime.datetime.now()
    mon=num_to_month(dt.month)
    thrs=getattr(emp1,mon)
    print(thrs)
    return render_template("attendencereport.html",res=emp,thrs=thrs)
