...
def Logout():
    ...
    emp=emphour.query.filter_by(username=uname).first()
    print(" THis is the uname",uname)
    ...
    em1=em.query.filter_by(username=uname).first()
...
def signupauth():
    ...
    # U_name=str(data.get("email"))
    # P_word=str(data.get("password"))
    U_name=str(data.get('U_name'))
    P_word=str(data.get('P_word'))
...
def attendence_report():
    ...
    return jsonify({"success": True, "res":emp_data,"thrs":thrs}),200
@app.route("/check",methods=["POST"])
def check():
    data=request.get_json()
    u_name_emp=str(data.get("emp"))
    mode= int(data.get("mode"))
    mon=str(data.get("month"))
    emp1=employee()

    users=User.query.all()
    print(" the mode bit is ",mode)
    if mode ==1:
        ...
        print(value)
        return jsonify({"success": True, "res":emp_data,"thrs":value,"mode":1}),200
    elif mode ==2:
        ...
        print(" the emp1 is ",emp_data )
        return jsonify({"success": True, "res":emp_data,"mode":2}),200
@app.route("/emphrs")
def  emphrs():
    users=User.query.all()
    l=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"]
    ...
    # the USers [<User 1>, <User 2>, <User 3>, <User 4>]
    return jsonify({"users": user_data, "months": l}), 200
@app.route("/depthrs")
...
def depthrsub():
    ...
    return jsonify({"success":True,"sum":sum,"usernames":usernames,"hrs":hrs})
